...
const fontNav = Kanit({ weight: "100", subsets: ["latin"]  })

export const NavBar = ({dataNav} : NavBarProps) => {
  const [toggleNav, setToggleNav] = useState(false)
  return (
    <header className={`${inter.className} relative z-40 flex items-center justify-between px-[30px] md:px-0 md:justify-around w-full py-[5px] bg-[#000000] opacity-90`}>
      <nav className={`flex flex-col md:flex-row justify-around md:justify-between w-full md:w-[55%]`}>
        <div className='flex justify-between'>
          <Link href="/">
            <Image 
              className='w-[110px] h-full hover:scale-110 transition-[0.5s]' 
        ...
        <ul className={`${fontNav} ${toggleNav ? "h-[200px]" : "h-0"} overflow-hidden md:overflow-visible duration-300 md:h-auto md:px-[30px] md:flex flex-col md:flex-row items-center justify-center md:justify-evenly md:gap-3 w-full md:w-[45%]`}>
      ...
      <div className='md:flex items-center hidden'>
        <a className="text-white text-[20px] gap-3 flex items-center hover:scale-110 transition-[0.5s]" href="">
